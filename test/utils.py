...
def create_uniform_particles(x_range :tuple[float, float], y_range :tuple[float, float], hd_range :tuple[float, float], N :int) -> np.ndarray:
    particles = np.empty((N, 3))
    particles[:, 0] = np.random.uniform(x_range[0], x_range[1], size=N)
    particles[:, 1] = np.random.uniform(y_range[0], y_range[1], size=N)
    particles[:, 2] = np.random.uniform(hd_range[0], hd_range[1], size=N) % 2*np.pi
    return particles
